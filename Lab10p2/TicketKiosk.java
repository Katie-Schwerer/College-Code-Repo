...
public class TicketKiosk
{
    ...
    public void purchaseTickets(Scanner keyboard)
    {
        ...
        if (!name.equals("exit"))
        {
            ...
            if (flight != null)
            {
                Ticket ticket = new Ticket(custom, flight);
                System.out.println("\n" + ticket + "\n");
            }
    ...
    public Flight readOneFlight(Scanner inFile) throws IOException
    {
        ...
        // File Number
        int fn = inFile.nextInt(); 

        inFile.nextLine();

        // Pilot File
        Pilot fpilot = new Pilot(inFile.nextLine(), inFile.nextLine());

        // File Date
        String fd = inFile.nextLine();

        // File Destination
        String fdest = inFile.nextLine();

        return new Flight(fn, fdest, fpilot, fd);
    }   
}
